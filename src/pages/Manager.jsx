...
/* ---- empty state ---- */
function EmptyState({ onAddCapoClick, onImportClick }) {
  return (
    <div className="card empty">
      <div className="empty__icon">🧭</div>
      <h3 className="empty__title">Inizia a costruire l’organigramma</h3>
      <p className="empty__text">
        Importa un file CSV/XLSX oppure crea subito un Capo. Potrai trascinare gli operai nelle squadre.
      </p>
      <div className="actions">
        <button className="btn primary" onClick={onImportClick}>📥 Importa file</button>
        <button className="btn" onClick={onAddCapoClick}>+ Aggiungi Capo</button>
      </div>
    </div>
  );
}
...
export default function Manager(){
  ...
  const inSource = org?.unassigned?.length || 0;
  const isEmpty = total === 0 && teamsCount === 0 && inSource === 0;
    ...
    <div className="space-y-4">
      ...
      {isEmpty ? (
        <EmptyState
          ...
          onImportClick={()=> {
            const hidden = document.querySelector('.topbar input[type="file"]');
            hidden?.click();
          }}
        />
      ) : (
        <>
          <div className="card flex items-center gap-2">
            ...
            <button className="btn ghost" onClick={handleClearAll}>Svuota tutto</button>
