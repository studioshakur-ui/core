...
export function editTeam(id, patch = {}) {
  ...
  if (i >= 0) {
    org.teams[i] = { ...org.teams[i], ...patch };
    saveOrg(org);
  }
  return org;
}

export function moveMember(memberId, teamId /* null => sorgente */) {
  const org = loadOrg();
  ...
  if (teamId) {
    ...
    if (t) {
      t.members = t.members || [];
      if (!t.members.includes(memberId)) t.members.push(memberId);
    }
    org.unassigned = (org.unassigned || []).filter((id) => id !== memberId);
  } else {
    org.unassigned = org.unassigned || [];
...
export function getMemberById(id) {
  const org = loadOrg();
  return (org.members || []).find((m) => m.id === id);
}
...
export function ensureMember(name, role) {
  const ex = findMemberByName(name);
  if (ex) return ex;
  return addMember({ name: _toTitle(name), role });
}

export function ensureTeamByCapoName(capoName) {
  const org = loadOrg();
  const capo = ensureMember(capoName, "Capo");
  // une équipe qui a déjà ce capo ?
  let team = (org.teams || []).find((t) => t.capo === capo.id);
  if (!team) {
    team = addTeam("capo");
    // déplacer le capo dans la colonne et le définir comme capo
    moveMember(capo.id, team.id);
    editTeam(team.id, { capo: capo.id });
  }
  return { team, capo };
}

export function unsetCapo(teamId) {
  const org = loadOrg();
  return editTeam(teamId, { capo: undefined });
}

export function addCapo(name) {
  const { team, capo } = ensureTeamByCapoName(name);
  return { team, capo };
}

export function deleteCapo(memberId) {
  return removeMember(memberId); // nettoie capo et références
}

// --- Nettoyage des faux noms déjà stockés (optionnel) ---
const _upperRatio = (s) => {
  const letters = s.replace(/[^A-Za-zÀ-ÿ]/g, "");
  if (!letters) return 0;
  const upp = letters.replace(/[^A-ZÀ-Ü]/g, "");
  return upp.length / letters.length;
};
const _STOP = [
  "APP","AREA","CABINA","CABINE","COLLEGAMENTI","COLLEGAMENTO","COMPLETAMENTI",
  "FUORI","MOTORE","PAX","QUADRO","MAGAZZINO","OFFICINA","TOT","PERSONE","COSTR",
  "TEST","CP","ELETTRICA","COMODINI","FINALE","NOTE"
];
const _isLikelyPerson = (s) => {
  const v = _norm(s);
  if (!v || /\d/.test(v)) return false;
  const parts = v.split(/\s+/).filter(Boolean);
  if (parts.length < 2) return false;
  if (_upperRatio(v) > 0.8) return false;
  if (_STOP.some((w) => v.toUpperCase().includes(w))) return false;
...
export function cleanseOrg() {
  const org = loadOrg();
  if (!org || !Array.isArray(org.members)) return org;
  ...
  for (const m of org.members) {
    const name = _norm(m?.name);
    if (_isLikelyPerson(name)) {
      newMembers.push(m);
      validIds.add(m.id);
    } else {
      suspects.push({ id: m.id, name, role: m.role });
    }
  }

  (org.teams || []).forEach((t) => {
    t.members = (t.members || []).filter((id) => validIds.has(id));
    if (!validIds.has(t.capo)) t.capo = undefined;
  });
  org.unassigned = (org.unassigned || []).filter((id) => validIds.has(id));
  org.members = newMembers;
  org.suspects = suspects;
  saveOrg(org);
  return org;
}
...
const orgStore = {
  ...
  addTeam, removeTeam, editTeam, moveMember,
  getMemberById,
  findMemberByName, ensureMember, ensureTeamByCapoName, unsetCapo, addCapo, deleteCapo,
  cleanseOrg,
};
